"""Signal-processing notebook tabs: Set Window, Feature Extraction, Deep Predict."""
...
from .config import FEATURE_EXTRACTION_DIR, MODELS_DIR, SCRIPT_DIR
from .ml import (
    ALL_SPECTRUM_PREFIXES,
    AVAILABLE_MODELS_DICT,
    PEAK_DEPENDENT_FEATURES,
    load_required_features,
    parse_feature,
...
DEFAULT_CONFIG = {
    ...
    # Filter (off by default here -- no filter_helper wired up)
    "ENABLE_FILTER": False,
    "FILTER_PARAMS": {},
    "FILTER_CHAIN": [],
...
# ---------------------------------------------------------------------------
# Module state (shared between the three tabs)
# ---------------------------------------------------------------------------
sp_loaded_df        = None
...
def build_signal_processing_tabs(parent_notebook, root_window):
    ...
    Returns a cleanup callable to invoke on application close.
    """
    root     = root_window
    notebook = parent_notebook

    # -----------------------------------------------------------------------
    # Font size -- applies to all widgets via the named system fonts
    # -----------------------------------------------------------------------
    ...
    def load_file():
        ...
        _initial_dir = os.path.join(SCRIPT_DIR, "temp_files")
        if not os.path.isdir(_initial_dir):
            _initial_dir = SCRIPT_DIR
    ...
    # -- Section: Peak-Based Fine Tuning -----------------------------------------------
    ...
    # -- Sliding Window / Center Peak params ----------------------------------------
    ...
    # -- Fine-Tuned Drop Detection params --------------------------------------------
    ...
    # -- Status label ------------------------------------------------------------------
    status_var = tk.StringVar(value="Ready -- load a parquet file to begin")
    ...
    def _model_paths(model_dir):
        """Return the preferred model file inside model_dir."""
        p1 = os.path.join(model_dir, "weighted_ensemble_model.joblib")
        p2 = os.path.join(model_dir, "elastic_net_model_pipeline.pkl")
    ...
    def _update_model_status(*args):
        ...
        if sel_mdl in AVAILABLE_MODELS_DICT:
            m_path = _model_paths(AVAILABLE_MODELS_DICT[sel_mdl])
    ...
    feat_status_var = tk.StringVar(value="Ready -- load a file and set trim window first")
    ...
    feat_class_var = tk.StringVar(value="—")
    ...
    def _update_feat_info():
        ...
        if trim_result is not None:
            s, e, dur = trim_result
            feat_trim_var.set(f"Trim: sample {s} -> {e}  ({dur:.4f} s)")
        else:
            feat_trim_var.set("No trim window -- run Preview Trim Window first")
        ...
        try:
            ...
            if len(trimmed) == 0:
                messagebox.showwarning("Empty Trim",
                                       "Trim returned an empty signal -- adjust parameters.")
                return

            duration_s = len(trimmed) / sr
            trim_result = (s, e, duration_s)
            status_var.set(
                f"Trim window ({tm}): sample {s} -> {e}  |  {duration_s:.4f} s"
            )
    ...
    def _draw_plot():
        ...
        for ax, (col, colour, ylabel) in zip(axes, channels):
            if col in cols:
                ...
                # Peak markers on the selected peak detection channel when First-to-Last
                if (col in ("AI0 (V)", "AI2 (V)") and has_trim
                        and detected_peaks_arr is not None
                        and len(detected_peaks_arr) > 0
                        and peak_channel_var.get() == col):
                    pk_abs = detected_peaks_arr + s_idx
                    ax.plot(pk_abs / sr,
                            sp_loaded_df[col].values[pk_abs],
    ...
    def _start_extraction():
        ...
        feat_status_var.set("Starting feature extraction…")
    ...
    def _cancel_extraction():
        ...
        feat_status_var.set("Cancelling…")
    ...
    def _run_extraction_thread(signal, sr, m_dir, m_path):
        ...
        try:
            ...
            _feat_progress(0, 1, "Importing libraries…")
            ...
            _feat_progress(0, 1, "Initialising extractor…")
            ...
            plan = {}   # filter_name -> {"configs": set, "base_needed": bool}
            ...
            all_extracted = {}   # final_feature_name -> value
            ...
            _feat_progress(n_calls, n_calls, "Building feature vector…")
            ...
            # --- Load model & predict ---
            _feat_progress(n_calls, n_calls, "Loading model…")
            register_pickle_classes()   # pickles reference __main__.WeightedEnsemble*
            ...
            _feat_progress(n_calls, n_calls, "Running prediction…")
